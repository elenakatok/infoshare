...
/*
  ...
  ⚠ THIS EXISTS BECAUSE ONE OBSERVED FAILURE WAS UNRECOVERABLE. On 2026-07-29 a run
  reported "229 passed, 1 FAILED". The run had been piped through `tail -6`, so the one
  `✗ FAIL:` line was cut before anyone read it, and the failure never reproduced in ~17
  subsequent runs. Which assertion failed is still unknown. A suite whose failures are
  only observable if someone happened to keep the output is a suite that cannot be
  debugged from a rare event, and rare events are the ones worth debugging.
  ...
  the bot land on either role across runs, which is what exercises BOTH strategy branches
  (retailer: 1 assertion, supplier: 2 — hence the 230/231 totals). Pinning it would hide a
  whole branch behind a stable-looking count. Keep the variation; capture the evidence.

  Writes with writeSync to a raw fd rather than a stream, so nothing is lost to buffering
  when the run ends in process.exit(). The emulator's own output already goes to
  e2e-emu.log; `npm run build` inherits the real fd and so is not mirrored here — its
  output is a compile, not evidence about an assertion.
*/
...
/*
  ...
  mock classroom is still holding port 5599.)
*/
