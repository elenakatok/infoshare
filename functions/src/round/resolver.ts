// ═══════════════════════════════════════════════════════════════════════════════
// ⚠ PLACEHOLDER_GAME payoff function (spawn Part 1).
// Replaced by the payoff slice, per Information_Sharing_Game_Specification_v1 §3.
//
...
export interface RoundInput {
  /** Retailer's message. Free — it need not match `state`. */
  signal: DrawnState
  /** Supplier's committed quantity. */
  quantity: number
  /** The truth Retailer saw and Supplier did not. */
  state: DrawnState
}
...
export function resolveRound(input: RoundInput, s: RoundSettings): RoundOutcome {
  ...
  return {
    ...
    sold,
    profits: {
      // Retailer is paid on what sells and bears none of the commitment cost — so Retailer
      // always prefers a larger quantity, whatever the true state. That asymmetry is
      // what gives the placeholder game a reason for Retailer to shade the signal, and
      // is the shape most cheap-talk stage games have.
      retailer: s.retailerRate * sold,
      supplier: s.supplierRate * sold - s.unitCost * input.quantity,
    },
  }
}
