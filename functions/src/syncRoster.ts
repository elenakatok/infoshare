...
const infoshareCallbackSecret = defineSecret('INFOSHARE_CALLBACK_SECRET')
const sharedSyncRoster = makeSyncRoster(infoshareGameDef)
const isEmu = () => process.env.FUNCTIONS_EMULATOR === 'true'

export const syncRoster = onCall(
  { cors: infoshareGameDef.corsOrigins, secrets: [infoshareCallbackSecret] },
  async (request: CallableRequest) => {
    ...
    try {
      ...
      const secret = String(devData?.['callback_secret'] ?? process.env.INFOSHARE_CALLBACK_SECRET ?? '')
      if (rosterUrl && secret) {
      ...
      console.error('[infoshare syncRoster] instructor_email denormalize failed (non-fatal):', err instanceof Error ? err.message : err)
    }
