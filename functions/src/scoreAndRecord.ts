...
function resolveCallbackConfig(data: Record<string, unknown>, isEmulator: boolean): { url: string; secret: string } {
  ...
  return {
    ...
    secret: (dev?.['callback_secret'] as string | undefined) ?? process.env.INFOSHARE_CALLBACK_SECRET ?? '',
  }
}

const def = infoshareGameDef
 ...
 */
export const scoreAndRecord = onCall({ cors: def.corsOrigins, secrets: [infoshareCallbackSecret] }, async (request) => {
