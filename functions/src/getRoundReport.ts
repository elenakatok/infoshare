...
// ═══════════════════════════════════════════════════════════════════════════════
// THE PER-ROUND REPORT — the data behind Tier 1b and Tier 3.
// ⚠ PLACEHOLDER_GAME row shape (spawn Part 1); replaced by the reports slice (§10).
//
...
export const getRoundReport = onCall({ cors: infoshareGameDef.corsOrigins }, async (request) => {
  const data = request.data as Record<string, unknown>
  const iid = await extractInstructorGameId(data, isEmu(), authHeaderOf(request))

  const snap = await admin.firestore()
    .collection('game_instances').doc(iid).collection('infoshare_round').get()
