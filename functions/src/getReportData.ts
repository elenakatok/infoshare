...
import { computeScoreBreakdown, infoshareGameDef } from './gameDefinition'
...
export const VALID_ROLES = new Set(infoshareGameDef.roles.roles.map(r => r.key))

// Text questions from prepDefaults — read once at module load (none graded here).
export const TEXT_QUESTIONS = (infoshareGameDef.prepDefaults ?? [])
...
export const getReportData = onCall({ cors: infoshareGameDef.corsOrigins }, async (request) => {
  ...
  try {
    ...
    return { ok: true as const, rows, questions: TEXT_QUESTIONS, schema: infoshareGameDef.outcomeSchema }
  } catch (err) {
