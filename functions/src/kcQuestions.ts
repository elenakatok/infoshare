// ═══════════════════════════════════════════════════════════════════════════════
// THE QUESTION BANK — ⚠ PLACEHOLDER_GAME content (spawn Part 1).
// Replaced by the KC slice, per Information_Sharing_KC_Questions_v1.
//
...
export const GRADED_QUESTIONS: Question[] = [
  gq('kc_signal_is_free', 1, 'no_requirement',
    'Retailer sees the true state of the round before sending a signal. Must Retailer\'s signal match it?',
    [
      { value: 'must_match', label: 'Yes — the signal is checked against the true state and rejected if it differs' },
      { value: 'no_requirement', label: 'No — Retailer may send either signal, whatever the true state is' },
      { value: 'only_first_round', label: 'Only in the first round; after that the signal is free' },
      { value: 'supplier_sees_both', label: 'It does not matter, because Supplier sees the true state anyway' },
    ],
    'The signal is unconstrained: Retailer may send either value regardless of the truth. ' +
    'Supplier sees only the signal at decision time — the true state becomes public once the ' +
  ...
  gq('kc_what_beta_sees', 2, 'signal_and_history',
    'At the moment Supplier commits a quantity, what can Supplier see?',
    [
      { value: 'state_only', label: 'The true state of this round, but not the signal' },
      { value: 'signal_and_history', label: 'Retailer\'s signal for this round, plus the full history of past rounds' },
    ...
    'Supplier decides on the signal and on the record of what happened before. The current ' +
